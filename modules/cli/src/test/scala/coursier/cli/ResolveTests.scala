...
object ResolveTests extends TestSuite {
  ...
  def sameLines(foundOrig: String, expectedOrig: String) = {
    val found = foundOrig.replace("\r\n", "\n")
    val expected = expectedOrig.replace("\r\n","\n")
  ...
  val tests = Tests {
    test("print what depends on") {
      ...
      sameLines(output, expectedOutput)
    }
    ...
    def homepath(root: String, path: String) = java.nio.file.Paths.get(root, path.split('/').toArray :_*)
    ...
    test("print results anyway") {
      ...
      val expectedOutput =
        s"""Error downloading ioi.get-coursier:coursier-core_2.12:1.1.0-M9
          |  not found: ${homepath("HOME", ".ivy2/local/ioi.get-coursier/coursier-core_2.12/1.1.0-M9/ivys/ivy.xml")}
    ...
    test("ignore binary scala version") {
      ...
      val expectedOutput =
        """org.scala-lang:scala-library:2.12.11:default
          |""".stripMargin
      sameLines(output0, expectedOutput)
    }
